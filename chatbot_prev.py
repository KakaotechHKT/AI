...
class ChatBot:
    ...
    def call_model(self, state: MessagesState):
        ...
        print(state["messages"])
        trimed_messages = self.trimmer.invoke(state["messages"])
        formatted_messages = self.remake_message(trimed_messages)
        response = self.model_with_tools.invoke(formatted_messages)

        state["messages"] = trimed_messages + [response]
        # add trimmer
